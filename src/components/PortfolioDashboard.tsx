          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <button
                ...
                onClick={() => {
                  window.location.href = '/transaction-history?status=pending';
                }}
              >
                <div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center mb-2">
                  <svg className="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="text-center">
              ...
              <div className="space-y-2">
                <a
                  href={wealthSpecialist.email ? `mailto:${wealthSpecialist.email}` : undefined}
                  className={`w-full bg-gray-100 hover:bg-gray-200 text-gray-900 py-2 px-4 rounded-lg text-sm font-medium transition-colors block text-center ${!wealthSpecialist.email ? 'opacity-50 cursor-not-allowed' : ''}`}
                  {...(!wealthSpecialist.email && { tabIndex: -1 })}
                  onClick={e => { if (!wealthSpecialist.email) e.preventDefault(); }}
                >
                  Email
                </a>
                <a
                  href={wealthSpecialist.phone ? `tel:${wealthSpecialist.phone}` : undefined}
                  className={`w-full bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-lg text-sm font-medium transition-colors block text-center ${!wealthSpecialist.phone ? 'opacity-50 cursor-not-allowed' : ''}`}
                  {...(!wealthSpecialist.phone && { tabIndex: -1 })}
                  onClick={e => { if (!wealthSpecialist.phone) e.preventDefault(); }}
                >
                  Call
                </a>
              </div>
